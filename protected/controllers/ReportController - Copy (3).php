		...
		if(isset($_GET['Cart']))
		{
			...
			$data = Yii::app()->db->createCommand()
			   ...
			   $data=$data->where("cart.time BETWEEN  '".$_GET['Cart']['status']." 00:00:00' AND '".date( "Y-m-d", strtotime( $_GET['Cart']['status']." +6 days " ) )." 23:59:00'");
    			...
    			->select('cart.id, cart.time,DATE_FORMAT(cart.time, "%Y-%m-%d") as date,history.productId,history.productName,sum(history.price) as hinumber')
   				->from('cart')
   				   ->join('history', 'cart.cartId=history.cartId');
...
//			    $dataweek=$dataweek->where("cart.status=0");
				...
				// month sales
				$dataweek = Yii::app()->db->createCommand()
    			->select('cart.id, cart.time,DATE_FORMAT(cart.time, "%Y-%m") as date,history.productId,history.productName,sum(history.price) as hinumber')
   				->from('cart')
   				   ->join('history', 'cart.cartId=history.cartId');
...
//			 	if($aprocount)
			 ...
			 foreach ($dataweek as $key => $value) {
			 	...
			 	array_push($weekdata,$value['hinumber']);
			 }
			}
