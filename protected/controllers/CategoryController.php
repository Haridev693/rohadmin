...
class CategoryController extends Controller
{
	...
	public function actionCreate()
	{
		...
		if(isset($_POST['Category']))
		{
            ...
            if(is_object($uploadedFile)&& get_class($uploadedFile)=== 'CUploadedFile')
            {
                ...
                $model->ImageUrl = $name_file;
            }
			if($model->save())
            {
                if(isset($uploadedFile)&& strlen($uploadedFile->size)>0)
                {
                    $uploadedFile->saveAs(Yii::app()->basePath.'/../images/category/'.$model->ImageUrl);
                }
            }
	...
	public function actionUpdate($id)
	{
		...
		{
			$model->attributes=$_POST['Category'];
            $_POST['Category']['ImageUrl']= $model->ImageUrl;
            $uploadedFile = CUploadedFile::getInstance($model,'ImageUrl');
            if(is_object($uploadedFile)&& get_class($uploadedFile)=== 'CUploadedFile')
            {
                $time_string = time();
			...
			if($model->save())
            {
                if(!empty($uploadedFile))
                {
                    if(isset($uploadedFile)&& strlen($uploadedFile->size)>0)
                    {
                        if(($model->ImageUrl)!= $oldImage && strlen($oldImage)>0)
                        {
	...
	{
		$oldImage = $this->loadModel($id)->ImageUrl;
